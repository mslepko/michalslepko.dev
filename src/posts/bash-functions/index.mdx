...
```
<code class="bash">function parse_hg_branch {
...
```
<code class="bash"># Based on http://dotfiles.org/~pseup/.bashrc
# use: extract <file>
function extract() {
    if [ -f "$1" ] ; then
...
```
<code class="bash"># source: http://stackoverflow.com/questions/1875305/command-line-csv-viewer
function csvpreview(){
...
```
<code class="bash"># useage files_perm 655
function files_perm(){
...
```
<code class="bash">function dir_perm(){
