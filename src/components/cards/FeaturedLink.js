...
function FeaturedLink({ link, thumb }) {
  ...
  let classes =
    "flex items-center w-full p-4 text-dark rounded-lg shadow-md dark:text-gray-400 flex-col transition-all duration-500 hover:bg-mint hover:scale-105 bg-clip-border shadow-3xl shadow-shadow-500 bg-white my-4 featured-link featured-link__" + link.name.split(' ')[0].toLowerCase()

  let thumbClass = "text-center thumbnail h-full w-full h-[250px] flex justify-center items-center my-4"
